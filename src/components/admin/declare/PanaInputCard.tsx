...
export function PanaInputCard() {
  ...
  return (
    <div
      id="pana-input-card"
      className={cn(
        "rounded-xl bg-card border border-border/60 border-t-2 p-4 transition-colors",
        valid ? "border-t-success" : pana.length === 3 ? "border-t-destructive" : "border-t-primary",
      )}
    >
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="text-[9px] uppercase tracking-[0.25em] text-primary">Step 2</div>
          <h2 className="font-display text-lg font-bold">Enter Pana Number</h2>
        </div>
      ...
      {disabled && (
        <div className="mb-3 rounded-md bg-muted/30 border border-border/60 p-2 text-xs text-muted-foreground text-center">
      ...
      <div className={cn("flex flex-col items-center", disabled && "opacity-40 pointer-events-none")}>
        <div className="text-[9px] uppercase tracking-[0.3em] text-muted-foreground mb-2">
        ...
        <div className="mt-2 text-xs text-muted-foreground font-mono tracking-widest min-h-4">
      ...
      {/* Inline reference */}
      {digit !== null && (
        <div className="mt-3 rounded-md border border-border/60 bg-card/60 px-2 py-1.5 text-[11px]">
      ...
      {[0, 1, 2].map((i) => (
        <div key={i} className="flex flex-col items-center">
          <input
            ...
            tabIndex={i === 0 ? 1 : 2 + i}
            className={cn(
              "h-14 w-14 rounded-lg bg-background text-center font-mono text-2xl font-bold",
          ...
          />
          <span className="mt-1 text-[8px] uppercase tracking-widest text-muted-foreground">Digit {i + 1}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center">
      <input
        ...
        placeholder="0 0 0"
        className={cn(
          "h-14 w-48 rounded-lg bg-background text-center font-mono text-2xl font-bold tracking-[0.4em] text-primary text-glow-gold",
  ...
  return (
    <div className="rounded-lg border border-success/30 bg-success/10 p-3">
      <div className="flex items-center gap-2 text-success font-bold text-sm">
        <Check className="h-3.5 w-3.5" /> Pana {pana} is VALID
        <Badge variant="outline" className={cn("ml-2 font-display text-[10px]", badge.className)}>
          {badge.label} · {type}
        </Badge>
      </div>
      <div className="mt-2 grid grid-cols-1 sm:grid-cols-[1fr_auto] items-center gap-3">
        <div className="text-xs text-muted-foreground">
          <div className="text-[9px] uppercase tracking-widest text-success/80">Digit Calculation</div>
  ...
  return (
    <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3">
      <div className="flex items-center gap-2 text-destructive font-bold text-sm">
        <X className="h-3.5 w-3.5" /> Pana {pana} is NOT a valid Matka pana
      </div>
      ...
      {suggestions.length > 0 && (
        <div className="mt-2">
          <div className="text-[9px] uppercase tracking-widest text-muted-foreground mb-1.5">Did you mean?</div>
          <div className="flex flex-wrap gap-2">
            {suggestions.map((s) => (
              <button
                key={s}
                onClick={() => onPick(s)}
                className="rounded-md border border-primary/40 bg-primary/10 text-primary px-2 py-0.5 text-xs font-mono hover:bg-primary/20 transition"
              >
